...
Timestamp::Timestamp(const std::chrono::system_clock::duration dur,
...
Timestamp::Timestamp(const std::string& formatted)
    : has_timezone_{false} {
    // 2017-08-25T13:03:35
    // 2017-08-25T13:03:35.123456789
    // 2017-08-25 13:03:35
    // 2017-08-25 13:03:35.123457
    static const std::regex expr{"([[:digit:]]{4})-"  // year
                                 "([[:digit:]]{2})-"  // month
                                 "([[:digit:]]{2})"  // day
                                 "[[:space:]T]"
                                 "([[:digit:]]{2}):"  // hours
    ...
    std::smatch match{};
    _POSTGRES_CXX_ASSERT(std::regex_match(formatted, match, expr),
...
std::string Timestamp::format() const {
    ...
    auto time = unixTime();
    tm   parts{};
    res.resize(has_timezone_ ? strftime(&res[0],
                                        res.size(),
                                        nanos.count() ? "%FT%T.000000000 %z" : "%FT%T %z",
                                        localtime_r(&time, &parts)) : strftime(&res[0],
                                                                               res.size(),
                                                                               nanos.count()
                                                                               ? "%FT%T.000000000"
                                                                               : "%FT%T",
